<x-app-layout>
    ...
    <script>
        ...
        function createTomSelect(selectElement) {
            ...
            // Recalculate based on updated option cache
            row.querySelector('.quantity-input').addEventListener('input', function() {
                const selectedValue = tom.getValue();
                const product = tom.options[selectedValue];
                ...
                row.querySelector('.purchase-price').value = (qty * mrp).toFixed(2);
                calculateTotal();
            });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            setTimeout(() => {
                const firstSelect = document.querySelector('.product-select')?.tomselect;
                if (firstSelect) {
                    firstSelect.focus();
                }
            }, 200);
        ...
        window.addEventListener('storage', async function(event) {
            if (event.key !== 'product_updated') return;

            const data = JSON.parse(event.newValue);
            const productId = data.id;

            document.querySelectorAll('.product-row').forEach(async row => {
                if (row.dataset.productId != productId) return;

                try {
                    const response = await fetch(`/products/${productId}/json`);
                    const product = await response.json();
                    const tomSelect = row.querySelector('.product-select').tomselect;

                    // Force update the option in TomSelect's internal options hash
                    tomSelect.updateOption(product.id, product);
                    tomSelect.refreshOptions(false);

                    const qty = parseFloat(row.querySelector('.quantity-input').value || 1);
                    row.querySelector('.purchase-price').value = (qty * parseFloat(product.mrp ||
                        0)).toFixed(2);
